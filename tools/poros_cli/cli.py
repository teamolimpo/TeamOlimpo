...
app = typer.Typer(
    name="hermes_cli",
    help="Poros CLI \u2014 Validazione e gestione ID del Team Olimpo.",
...
def _output_text_handoff(hv) -> None:
    ...
    panel = Panel(
        "\n".join(lines),
        title=f"poros-cli validate handoff",
...
def handoff_create(
    ...
    mittente: str = typer.Option("poros", "--from", help="Mittente (default: poros)."),
...
def report(
    ...
    lines: list[str] = []
    lines.append("  [bold]Poros CLI \u2014 Report stato[/bold]")
    lines.append("")

    t = result["tasks"]
    open_ids = [
        f"{x['id']}"
        for x in t["by_agent"].get("poros", [])
...
def diff(
    ...
    if _json_state["json"]:
        ...
        panel = Panel(
            "\n".join(lines),
            title="poros-cli diff scratchpad",
...
def stats(
    ...
    panel = Panel(
        "\n".join(lines),
        title=f"poros-cli stats{prefix}",
        border_style="blue",
    )
    out_console.print(panel)
