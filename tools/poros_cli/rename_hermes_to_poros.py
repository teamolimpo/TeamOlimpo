#!/usr/bin/env python3
"""Esegue il rename massivo di "Poros" in "Poros" su tutto il codebase.
...
Modalità:
  ...
  --rollback   Ripristina tutto dai backup in .poros-rename-backup/
...
BACKUP_ROOT = PROJECT_ROOT / ".poros-rename-backup"

LOG_FILE = PROJECT_ROOT / ".poros-rename-backup" / "rename.log"

# FASE 1: file/directory da rinominare (src → dst relativi a PROJECT_ROOT)
RENAME_MAP: list[tuple[str, str]] = [
    (".opencode/agents/poros.md",          ".opencode/agents/poros.md"),
    ("Team/Members/poros.md",              "Team/Members/poros.md"),
    (".claude/agents/poros.md",            ".claude/agents/poros.md"),
    ("tools/hermes_cli",                    "tools/poros_cli"),          # merge (target already exists)
    ("Team/SOPs/poros-orchestration-methodology.md",
     "Team/SOPs/poros-orchestration-methodology.md"),
    ("Library/System/Poros",               "Library/System/Poros"),
]
...
REPLACE_PAIRS: list[tuple[str, str]] = [
    ("Poros", "Poros"),
    ("poros", "Poros"),  # ATTENZIONE: sostituiamo 'poros' con 'Poros' solo
    # quando la parola esatta è 'poros' (lowercase).
...
# (ad esempio il nome del tool 'poros' in contesti di routing)
...
CUSTOM_REPLACEMENTS: dict[str, list[tuple[str, str]]] = {
    "opencode.json": [
        ('"default_agent": "poros"', '"default_agent": "poros"'),
        (
            '"SYNAPSIS_DB_PATH": "lib/System/Poros/synapsis.db"',
            '"SYNAPSIS_DB_PATH": "Library/System/Poros/synapsis.db"',
        ),
    ],
    ".mcp.json": [
        (
            '"SESSION_DB_PATH": "lib/System/Poros/session.db"',
...
def _build_regex_patterns() -> list[tuple[re.Pattern, str]]:
    """Build regex patterns for Poros → Poros replacements.
    ...
    # "Poros" → "Poros" (word boundary, capitalized)
    patterns.append((re.compile(r"\bHermes\b"), "Poros"))

    # "poros" → "poros" (word boundary, lowercase) — solo se non è parte di
    # una parola più lunga come "Hermione"
    patterns.append((re.compile(r"\bhermes\b"), "poros"))

    # "POROS" → "POROS" (word boundary, uppercase)
    patterns.append((re.compile(r"\bHERMES\b"), "POROS"))
...
def _process_file_text(
    ...
    Steps:
      ...
      3. Apply regex replacements (Poros/poros → Poros/poros)
...
def _process_whitepaper(file_path: Path, dry_run: bool) -> bool:
    ...
    In whitepapers, "Poros" may appear as:
...
def _rollback(dry_run: bool) -> int:
    """Rollback all changes by restoring from .poros-rename-backup/.
...
def _print_report() -> None:
    ...
    print()
    print("=" * 60)
    print("  REPORT — Poros → Poros Rename")
    print("=" * 60)
...
def main() -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    parser = argparse.ArgumentParser(
        description='Esegue il rename massivo "Poros" → "Poros" su tutto il codebase.',
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--rollback",
        action="store_true",
        help="Ripristina tutto dai backup in .poros-rename-backup/.",
    )
    parser.add_argument(
        "--whitepaper",
        action="store_true",
    ...
    logger.info("=== Poros → Poros Rename Tool ===")
