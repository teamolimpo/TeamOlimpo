...
Define the standard structure, required files, naming conventions, and Synapsis entity metadata for all projects under `Library/projects/`. A consistent project layout ensures that `search("project-name")` returns structured results — status, phase, health — rather than scattered FTS5 snippets.
...
**Does not apply to:** Files outside `Library/projects/` (e.g., `Library/Wiki/`, `Library/Handoff/`, `Team/`). One-off notes, scratch files, or personal documents within a project directory that do not follow the naming convention SHOULD be placed in a `log/` or `docs/` subdirectory.
...
| **Chunk Indexer** | On re-index, parses frontmatter from `README.md` and `STATUS.md`, updates Synapsis entity metadata accordingly. |
...
health: on_track
last_updated: 2026-06-05     # ISO date
---
```
...
- `OLM-SOP-001` — SOP Format (`Team/SOPs/OLM-SOP-001-sop-format.md`)
- Synapsis entities table: `tools/synapsis/store.py` lines 186–197
- Synapsis entity_search: `tools/synapsis/store.py` lines 2883–2924
- Synapsis get_or_create_entity: `tools/synapsis/store.py` lines 1597–1641
- Proteo research: `Library/System/proteo/2026-06-05_project-entity-research.md`
- Memory Bank pattern (Cursor/Claude Code) — external reference for file-based project state

## Revision History

| Version | Date | Author | Description of Change |
|---------|------|--------|----------------------|
| v1.0 | 2026-06-05 | Poros | Initial version — based on Proteo research and analysis of 15+ systems |
